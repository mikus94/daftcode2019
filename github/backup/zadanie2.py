...
class Car:
    ...
    def __init__(self, brand, tank_capacity, tanked_fuel):
        ...
        if tanked_fuel > tank_capacity:
            raise RuntimeError('Tanked fuel exceeds tank capacity!')
        # capacity should be > 0
        if tank_capacity <= 0:
            raise RuntimeError('Tank capacity should be positive integer!')
        # tanked fuel should be >= 0
        if tanked_fuel < 0:
            raise RuntimeError('Tanked fuel should be positive integer!')
    ...
    def fill_tank(self, **kwargs):
        ...
        except (RuntimeError, ValueError) as emsg:
            msg = (
                "Following error caused in fill_tank method. "
            )
            logging.error(msg + str(emsg))
            exit(0)
